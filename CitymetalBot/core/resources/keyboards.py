...
def get_keyboard(key, language='ru') -> Union[ReplyKeyboardRemove, ReplyKeyboardMarkup, InlineKeyboardMarkup]:
    if key == 'remove':
        ...
        return _create_keyboard([[get_string('languages.ru', language)]], one_time=True)
    elif key == 'menu':
        keyboard = [
...
def get_vacation_confirm_keyboard(language: str) -> InlineKeyboardMarkup:
    keyboard = [
        [InlineKeyboardButton(get_string('confirm', language), callback_data='confirm')],
        [InlineKeyboardButton(get_string('go_back', language), callback_data='back')]
    ]
    return InlineKeyboardMarkup(keyboard)
...
def get_parent_categories_keyboard(categories: list, language: str) -> InlineKeyboardMarkup:
    keyboard = []
    categories = sorted(categories, key=itemgetter(language + '_title'))
    for category in categories:
        keyboard.append([InlineKeyboardButton(category.get(language + '_title'), callback_data='categories:'+str(category.get('id')))])
    keyboard.append([InlineKeyboardButton(get_string('go_back', language), callback_data='category:to_menu')])
    return InlineKeyboardMarkup(keyboard)
...
def get_units_keyboard(language: str) -> InlineKeyboardMarkup:
    keyboard = [
        [InlineKeyboardButton(get_string('units.square_meters', language), callback_data='0')],
        [InlineKeyboardButton(get_string('units.tonns', language), callback_data='1')],
        [InlineKeyboardButton(get_string('units.meters', language), callback_data='2')],
        [InlineKeyboardButton(get_string('units.kilograms', language), callback_data='3')],
        [InlineKeyboardButton(get_string('units.pieces', language), callback_data='4')],
        [InlineKeyboardButton(get_string('go_back', language), callback_data='back')]
    ]
    return InlineKeyboardMarkup(keyboard)
...
def get_catalog_keyboard(parent_categories: list, language: str) -> InlineKeyboardMarkup:
    ...
    parent_categories = sorted(parent_categories, key=itemgetter(language + '_title'))
    ...
    keyboard = []
    # Разбить кнопки в столбей
    for item in parent_categories_buttons:
